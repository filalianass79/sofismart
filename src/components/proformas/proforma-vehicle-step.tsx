...
export function ProformaVehicleStep({ onVehicleSelected }: { onVehicleSelected: () => void }) {
  const { watch, setValue, formState: { errors } } = useFormContext<ProformaWizardValues>();
  ...
  function selectVehicle(v: VehicleHit) {
    ...
    if (v.targetSalePrice) setValue("priceHT", v.targetSalePrice);
    onVehicleSelected();
  }
    ...
    <section className="space-y-5 rounded-xl border border-navy-950/10 bg-white p-6 shadow-sm">
      <header>
        ...
        <p className="mt-1 text-sm text-navy-500">
          Véhicules disponibles — la proforma ne modifie pas le statut stock.
        </p>
      </header>
